...
class Apis extends CI_Model
{
	...
	function search_exams_by_time($from, $to) {
		if($this->session->userdata('session_id')) {
			if($this->apis->check_user_role() == 'admin') {
				$query = $this->db->query('SELECT DISTINCT * FROM student_records a, student s WHERE (a.student_ic = s.ic) AND (DATE(a.exam_date) BETWEEN "'.$from.'" AND "'.$to.'") ORDER BY -DATE(a.exam_date)');
				if ($query->num_rows() > 0) return $query->result_array();
			} else {
				$op_branch_id = $this->apis->get_user_branch_id();
				$query = $this->db->query('SELECT DISTINCT * FROM student_records a, student s WHERE (a.student_ic = s.ic) AND (DATE(a.exam_date) BETWEEN "'.$from.'" AND "'.$to.'") AND (a.branch_id = "'.$op_branch_id.'") ORDER BY -DATE(a.exam_date)');
				if ($query->num_rows() > 0) return $query->result_array();
			}
		}
		return NULL;	
		...
		if($this->session->userdata('session_id')) {
			if($this->apis->check_user_role() == 'admin') {
				if($class_code == "") {
					...
					$query = $this->db->query('SELECT DISTINCT * FROM ato a, student s WHERE (a.ic = s.ic) AND ((a.pre_post = "PRE") OR ((a.pre_post = "POST") AND (a.post_change_date = "YES"))) AND (DATE(a.exam_date) BETWEEN "'.$from.'" AND "'.$to.'") GROUP BY a.id ORDER BY -DATE(a.exam_date)');
					if ($query->num_rows() > 0) return $query->result_array();
				} else {
				...
				if($class_code == "") {
					// search ato info by time only (pre), if post ato change date, this ato also can be found
					$query = $this->db->query('SELECT DISTINCT * FROM ato a, student s WHERE (a.ic = s.ic) AND ((a.pre_post = "PRE") OR ((a.pre_post = "POST") AND (a.post_change_date = "YES"))) AND (DATE(a.exam_date) BETWEEN "'.$from.'" AND "'.$to.'") AND (a.branch_id = "'.$op_branch_id.'") GROUP BY a.id ORDER BY -DATE(a.exam_date)');
					if ($query->num_rows() > 0) return $query->result_array();
				} else {
					// search ato info by time and class (post), info post ato change date, this ato also cannot be found
	...
	function search_atos_by_time_download($from, $to, $class_code, $location, $slot) {
		if($this->session->userdata('session_id')) {
			if($this->apis->check_user_role() == 'admin') {
				...
				if($class_code == "") {
					// search ato info by time only (pre), if post ato change date, this ato also can be found
					$query = $this->db->query('SELECT DISTINCT a.exam_date, a.pre_post, "x", "xx", "xxx", "Etest Training", a.attendance, "N", "Waiting for the result", "198400164G", SUBSTRING(a.el, 1, 1), SUBSTRING(a.er, 1, 1), SUBSTRING(a.en, 1, 1), SUBSTRING(a.es, 1, 1), SUBSTRING(a.ew, 1, 1), s.salutation, s.ic_type, s.ic, "00/00/0000",s.lastname, s.firstname, s.othername, s.gender, DATE_FORMAT(s.birthday,"%d/%m/%Y"), s.age, s.citizenship, s.nationality, s.race, s.cn_level, s.edu_level, UCASE(SUBSTRING(s.lang, 1, 3)), s.block, s.street, s.floor_unit_no, s.building, s.postcode, s.tel, "test@changchun.edu.sg", s.emp_status, s.company_type, s.company_name, s.company_reg_no, s.industry, s.designation, s.salary_range FROM ato a, student s WHERE (a.ic = s.ic) AND ((a.pre_post = "PRE") OR ((a.pre_post = "POST") AND (a.post_change_date = "YES"))) AND (a.exam_location = "'.$location.'") AND (a.exam_time = "'.$slot.'") AND (DATE(a.exam_date) BETWEEN "'.$from.'" AND "'.$to.'") AND (a.branch_id = "'.$op_branch_id.'") GROUP BY a.id ORDER BY -DATE(a.exam_date)');
					if ($query->num_rows() > 0) return $query->result_array();
				} else {
					// search ato info by time and class (post), info post ato change date, this ato also cannot be found
					$query = $this->db->query('SELECT DISTINCT a.exam_date, a.pre_post, c.code, DATE_FORMAT(c.start_date,"%d/%m/%Y"), DATE_FORMAT(c.end_date,"%d/%m/%Y"), c.type, a.attendance, "N", c.level, "198400164G", SUBSTRING(a.el, 1, 1), SUBSTRING(a.er, 1, 1), SUBSTRING(a.en, 1, 1), SUBSTRING(a.es, 1, 1), SUBSTRING(a.ew, 1, 1), s.salutation, s.ic_type, s.ic, "00/00/0000",s.lastname, s.firstname, s.othername, s.gender, DATE_FORMAT(s.birthday,"%d/%m/%Y"), s.age, s.citizenship, s.nationality, s.race, s.cn_level, s.edu_level, UCASE(SUBSTRING(s.lang, 1, 3)), s.block, s.street, s.floor_unit_no, s.building, s.postcode, s.tel, "test@changchun.edu.sg", s.emp_status, s.company_type, s.company_name, s.company_reg_no, s.industry, s.designation, s.salary_range FROM ato a, student s, class c, student_class sc WHERE (a.ic = s.ic) AND (c.code = "'.$class_code.'") AND (c.class_id = sc.class_id) AND (sc.student_id = s.student_id) AND (a.pre_post = "POST") AND (a.post_change_date = "NO") AND (a.exam_location = "'.$location.'") AND (a.exam_time = "'.$slot.'") AND (DATE(a.exam_date) BETWEEN "'.$from.'" AND "'.$to.'") AND (a.branch_id = "'.$op_branch_id.'") ORDER BY -DATE(a.exam_date)');
					if ($query->num_rows() > 0) return $query->result_array();
				}
			}
		}
